...
import { getItemAndIndexFromArray, getSortedArrayOfObject } from './lib/utils.js';
import { CART_PAGE, CHECKOUT_PAGE, COURSES_PAGE } from './constants/page.const.js';
...
const app = Vue.createApp({
	...
	methods: {
		addOneItemToCart: function (id) {
			...
			const { data: cartItem, index: cartItemIndex } = this.getCartItemAndIndexById(lesson.id);

			if (!cartItem) {
				this.cartItems.push({ ...lesson, quantity: 1 });
			} else {
				this.cartItems[cartItemIndex] = { ...cartItem, quantity: cartItem.quantity + 1 };
			}
